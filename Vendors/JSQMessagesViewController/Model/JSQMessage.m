...
+(instancetype)messageWithSenderId:(NSString *)senderId displayName:(NSString *)displayName text:(NSString *)text {
    return [[JSQMessage alloc] initWithSenderId:senderId senderDisplayName:displayName setId:0 date:[NSDate date] text:text];
}
                           ...
                           media:(id<JSQMessageMediaData>)media
{
    ...
    self = [self initWithSenderId:senderId senderDisplayName:senderDisplayName setId:setId date:date isMedia:YES];
    if (self) {
        _media = media;
    }
    return self;
