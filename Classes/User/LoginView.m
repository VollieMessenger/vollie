...
- (void)viewDidLoad
{
    _loginButton.backgroundColor = [UIColor volleyFamousGreen];
...
- (IBAction)actionLogin:(id)sender
{
    ...
    if (![phoneNumber isEqualToString:@"8675309665"])
    {
	...
	if (phoneNumber.length != 0)
	{
		...
		[PFUser logInWithUsernameInBackground:phoneNumber password:phoneNumber block:^(PFUser *user, NSError *error)
		{
			if (user && !error)
			{
                ...
                if (!fullName.length)
                {
                    [ProgressHUD showError:@"Phone number is new, Please Register"];
    ...
    //       Make sure to set verified to 0, otherwise they could login.
            ...
            else
            {
                ...
                {
                    [ProgressHUD showError:@"Network Error" Interaction:1];
                }
            }
		}];
	}
